...
% Parameters are given in international units (the results are not good for
% some reason)
...
Rm=8e6;      %membrane resistance (Ohm)
Cm=1e-6;     %membrane capacitance (F)
V0=-52e-3;   %resting potential (V)
Vthr=-45e-3; %threshold potential (V)
Vmax=20e-3;  %spike peak (V)
Vmin=-72e-3; %spike undershoot (V)
Tap=2e-3;    %AP length (s)
Ipsc=5e-9;   %PSC amplitude (A)
Tpsc=5e-3;   %half-life of PSC decay (s)
Fn=5;        %noise frequency (Hz)
Fs=120;      %sensory input frequency (Hz)
h=.1e-3;     %step size (s)
time=0:h:sum(T); %time axis (s)
le=24e-3;    %boxcar length (s)
...
[S,STM,turn]=StiMA(a,vmis,stim,N,epg,T,h,ANG); %stimulus matrix and angle (deg)
...
for t=1:L-1
    ...
    V(i0,t+1)=V(i0,t)+dVdt(V(i0,t))*h+normrnd(0,3e-10,size(i0))'; %graded potential (V)
    V(i1,t+1)=AP.V(k(i1)); %action potential (V)
    k(i1)=k(i1)+1;

    GP2AP=i0(V(i0,t+1)>=Vthr); %neurons triggering an AP
    V(GP2AP,t+1)=Vthr;
    m=min(PC.t,L-t); %template exceeding time dimension is removed
    Iout(GP2AP,t+1:t+m)=Iout(GP2AP,t+1:t+m)+PC.I(1:m); %post-synaptic (output) current (A)
    j(GP2AP)=1;
...
for j=1:length(stim) %"STM" is built for each type of stimulus
    ...
    elseif stim(j)==5 %periodic
        P=.1;D=.05; %periodicity and duration (s)
...
Vin=V0*ones(N,L);
Iin=zeros(N,L);
event=(rand(N,L)<O*Fn*h)+(rand(N,L)<S*Fs*h); %0 = no AP / 1 = AP
...
AP.V=[V1(2:end) V2(2:end)];AP.t=length(AP.V); %action potential (V)
end

%% Post-synaptic current template
function[PC]=PstCr(Ipsc,Tpsc,h)
t=0:h:2e-3+7*Tpsc; %timespan (s)
SIN=sin(t(t<=2e-3)*pi/2-pi/2);
EXP=2.^(-(t(t>=2e-3)-2e-3)/Tpsc);
I1=Ipsc*(SIN-min(SIN))/(max(SIN)-min(SIN));
I2=Ipsc*(EXP-min(EXP))/(max(EXP)-min(EXP));
PC.I=[I1(2:end) I2(2:end)];PC.t=length(PC.I); %post-synaptic current (A)
end
